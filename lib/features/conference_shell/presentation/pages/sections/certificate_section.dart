        ...
        state.maybeWhen(
          generated: (cert) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(content: Text("Certificate generated! Save to gallery?")),
            );
          },
          saved: () {
            ScaffoldMessenger.of(context).showSnackBar(
      ...
      child: BlocBuilder<CertificateBloc, CertificateState>(
        builder: (context, state) {
          ...
          if (user == null || conference == null) {
            return const Center(child: Text("Please login and select a conference"));
          }
          ...
          return Center(
            child: Column(
              ...
              children: [
                ...
                Text("Your Certificate", style: TextStyle(fontSize: 20.sp, fontWeight: FontWeight.bold)),
                SizedBox(height: 10.h),
                Text("Generate your official participation certificate", textAlign: TextAlign.center),
                SizedBox(height: 30.h),
                state.maybeWhen(
                  loading: () => const CircularProgressIndicator(),
                  generated: (cert) => ElevatedButton(
                    onPressed: () {
                      context.read<CertificateBloc>().add(CertificateEvent.save(cert.filePath));
                    },
